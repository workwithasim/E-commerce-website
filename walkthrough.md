...
17. For a COD delivery, confirm the rider cannot complete it before recording collection. Record the exact server total, confirm duplicate recording is rejected, then move it through pending settlement and settled in the admin order detail. Confirm support-only staff cannot settle it and accountability fields never appear in the customer order response.
18. Open the admin Audit Logs tab. Filter by date, actor role, branch, action, entity type, and entity ID; inspect previous/new state; confirm customers and support-only staff are denied. API/database authorization is verified; visual and keyboard behavior remains pending.
19. Change a branch open state, product price/availability, create a voucher, and update tenant settings. Confirm each mutation produces one audit record with actor and previous/new values, and verify password/token/secret fields are absent.
