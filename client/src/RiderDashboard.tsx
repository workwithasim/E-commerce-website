...
export const RiderDashboard: React.FC<{ user: AuthUser; onLogout: () => void }> = ({ user, onLogout }) => {
  ...
  return <main className="container page-content"><div className="store-controls"><div><h1>{tenant?.name} deliveries</h1><p>{user.name}</p></div><button className="add-card-btn" disabled={busy} onClick={async () => { setBusy(true); try { const r = await api.setRiderAvailability(!available); setAvailable(r.isAvailable); } catch (e: any) { setError(e.message); } finally { setBusy(false); } }}>{available ? 'Available · go offline' : 'Offline · go available'}</button><button onClick={() => { stop(); onLogout(); }}>Sign out</button></div>
    ...
    {!shown.length && <p>No {tab} deliveries.</p>}<div className="products-grid">{shown.map(d => <article className="tracking-card" key={d.id}><h2>{d.order?.orderNumber}</h2><p className="status-badge">{d.status.replace(/_/g,' ')}</p><p>{d.order?.customerName}</p><p><a href={`tel:${d.order?.customerPhone}`}>{d.order?.customerPhone}</a></p><p>{d.order?.deliveryAddress}</p><p>Collect: {settings?.currencySymbol} {d.order?.total}</p>{d.order?.codRecord && <p>COD: {d.order.codRecord.status.replace(/_/g, ' ')}</p>}<ul>{d.order?.items.map((i,n) => <li key={n}>{i.quantity} × {i.productName}</li>)}</ul>{d.status === 'ON_THE_WAY' && d.order?.codRecord?.status === 'PENDING_COLLECTION' && <button className="checkout-action-btn" disabled={busy} onClick={async () => { setBusy(true); setError(''); try { await api.collectCod(d.orderId, d.order!.codRecord!.expectedAmount); await load(); } catch (e: any) { setError(e.message); } finally { setBusy(false); } }}>Confirm exact COD collected</button>}{transition[d.status] && <button className="checkout-action-btn" disabled={busy || (d.status === 'ON_THE_WAY' && d.order?.codRecord?.status === 'PENDING_COLLECTION')} onClick={() => update(d)}>{transition[d.status].label}</button>}{settings?.riderTrackingEnabled !== false && ['PICKED_UP','ON_THE_WAY'].includes(d.status) && <button className="checkout-action-btn" onClick={() => tracking === d.id ? stop() : start(d.id)}>{tracking === d.id ? 'Stop sharing location' : 'Share live location'}</button>}{!['DELIVERED','CANCELLED'].includes(d.status) && <button onClick={async () => { try { setChatOrderId(d.orderId); setMessages(await api.getOrderMessages(d.orderId)); } catch (e: any) { setError(e.message); } }}>Order chat</button>}{chatOrderId === d.orderId && <div><h3>Customer chat</h3>{messages.map(message => <p key={message.id}><strong>{message.type === 'SYSTEM' ? 'System' : message.sender?.name || 'Customer'}:</strong> {message.message}</p>)}<form onSubmit={async event => { event.preventDefault(); if (!messageText.trim()) return; await api.sendOrderMessage(d.orderId, messageText); setMessageText(''); setMessages(await api.getOrderMessages(d.orderId)); }}><input aria-label="Message customer" value={messageText} onChange={event => setMessageText(event.target.value)}/><button>Send</button></form></div>}{tracking === d.id && <p role="status">{lastUpdate ? `Location sent at ${lastUpdate}` : 'Waiting for device location…'}</p>}</article>)}</div>
  </main>;
};
