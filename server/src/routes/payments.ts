...
router.post('/cod/:orderId/collect', async (req, res) => {
  try {
    ...
    if (isRider) {
      ...
      if (order) record = await prisma.codRecord.findUnique({ where: { orderId: order.id } });
    }
    ...
    const updated = await prisma.$transaction(async tx => {
      ...
      await tx.auditLog.create({ data: { tenantId: req.tenant!.id, userId: req.user!.id, action: 'COD_COLLECTED', entity: 'CodRecord', entityId: record.id, oldValue: 'PENDING_COLLECTION', newValue: JSON.stringify({ status: nextStatus, collectedAmount, differenceAmount }) } });
...
router.patch('/cod/:orderId/settlement', requirePermission('payments.manage'), async (req, res) => {
  try {
    const scope = await orderScope(req.user!, req.tenant!.id);
    const record = await prisma.codRecord.findFirst({ where: { tenantId: req.tenant!.id, orderId: req.params.orderId, order: scope } });
    ...
    const updated = await prisma.$transaction(async tx => {
      ...
      await tx.auditLog.create({ data: { tenantId: req.tenant!.id, userId: req.user!.id, action: 'COD_SETTLEMENT_CHANGED', entity: 'CodRecord', entityId: record.id, oldValue: record.status, newValue: JSON.stringify({ status, receivedAmount: roundedReceived, differenceAmount, reason: disputeReason || undefined }) } });
      return tx.codRecord.findUniqueOrThrow({ where: { id: record.id }, include: codInclude });
    });
    getIO().to(`order:${record.orderId}`).emit('payment:updated', updated);
    res.json({ success: true, data: updated });
