...
router.patch('/:id/status', requireRole(staffRoles), async (req, res) => {
  try {
    const scope = await orderScope(req.user!, req.tenant!.id);
    ...
    if (!canTransitionOrder(current.status, status, current.orderMode, req.user!.role)) return res.status(409).json({ success: false, error: { message: 'This order transition is not allowed' } });
